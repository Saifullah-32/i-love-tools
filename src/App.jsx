...
const ToolHeader = ({ tool, navigate }) => ( 
  <motion.div initial={{opacity:0, x:-20}} animate={{opacity:1, x:0}} className="tool-header"> 
...
const RelatedTools = ({ currentTool, navigate }) => { 
  ...
  return ( 
    <motion.div initial={{opacity:0}} whileInView={{opacity:1}} viewport={{once:true}} style={{marginTop: '3rem', paddingTop: '2rem', borderTop: '1px solid var(--border)'}}> 
      ...
      <div className="responsive-grid"> {related.map(t => <ToolCard key={t.id} tool={t} navigate={navigate} />)} </div> 
    </motion.div> 
  ); 
};
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'home' && (
              <motion.div key="home" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.3 }} className="home-dashboard">
                <motion.div initial={{opacity: 0, y: -20}} animate={{opacity: 1, y: 0}} className="home-hero">
