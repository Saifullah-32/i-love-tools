...
const popularToolIds = ['videditor', 'image', 'json-ts', 'aes-encrypt', 'pdfgen', 'audioedit'];
...
export default function App() {
  ...
  const convertJsonToTs = () => { try { const obj = JSON.parse(jsonToTsInput); let ts = 'export interface GeneratedInterface {\n'; for (let k in obj) ts += `  ${k}: ${Array.isArray(obj[k]) ? 'any[]' : typeof obj[k]};\n`; setTsOutput(ts + '}'); showToast('Converted successfully!'); } catch (e) { setTsOutput('Error: Invalid JSON format'); showToast('Invalid JSON Format', 'error'); } };
  
  const [cronInput, setCronInput] = useState('0 12 * * 1-5'); const [cronResult, setCronResult] = useState(''); 
  const translateCron = () => { try { setCronResult(cronstrue.toString(cronInput)); } catch (e) { setCronResult('Error: Invalid Cron Expression'); showToast('Invalid Cron', 'error'); } };
  ...
  const testRegex = () => { try { const re = new RegExp(regexPattern, 'g'); const matches = regexText.match(re); setRegexResult(matches ? matches.join(', ') : 'No matches found.'); } catch(e) { setRegexResult('Error: Invalid Regex Pattern'); showToast('Invalid Regex', 'error'); } };
  ...
  const handleCompressImage = async () => { if (!originalImage || !targetSize || targetSize <= 0) { showToast('Please enter a valid size', 'error'); return; } setCompressing(true); try { const sizeInMB = targetUnit === 'KB' ? targetSize / 1024 : Number(targetSize); const options = { maxSizeMB: sizeInMB, maxWidthOrHeight: 4000, useWebWorker: true }; const compressedFile = await imageCompression(originalImage, options); setCompressedImage(compressedFile); showToast('Image Compressed!'); } catch (error) { showToast('Failed to compress. Try larger target.', 'error'); } finally { setCompressing(false); } };
  ...
  const formatJson = () => { try { setJsonOutput(JSON.stringify(JSON.parse(jsonInput), null, 2)); showToast('JSON Formatted'); } catch (e) { setJsonOutput('Error: Invalid JSON'); showToast('Invalid JSON', 'error'); } };
  ...
  const compressDocs = async () => { if (zipFiles.length === 0) return; setZipping(true); const zip = new JSZip(); zipFiles.forEach(file => zip.file(file.name, file)); const content = await zip.generateAsync({ type: 'blob' }); setZipUrl(URL.createObjectURL(content)); setZipping(false); showToast('ZIP Ready!'); };
  ...
  const decodeJwt = () => { try { setJwtData(JSON.stringify(JSON.parse(atob(jwt.split('.')[1].replace(/-/g, '+').replace(/_/g, '/'))), null, 2)); } catch (e) { setJwtData('Invalid JWT Format'); showToast('Invalid JWT', 'error'); } };
  ...
  const downloadVideo = () => { const blob = new Blob(recordedChunks, { type: 'video/webm' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'screen-recording.webm'; a.click(); setRecordedChunks([]); showToast('Video Downloaded'); };
  ...
  const convertSvg = () => { const blob = new Blob([svgInput], { type: 'image/svg+xml;charset=utf-8' }); const img = new window.Image(); img.onload = () => { const canvas = document.createElement('canvas'); canvas.width = img.width; canvas.height = img.height; canvas.getContext('2d').drawImage(img, 0, 0); setPngUrl(canvas.toDataURL('image/png')); showToast('Converted to PNG'); }; img.src = URL.createObjectURL(blob); };
  ...
  const handleUrlTransform = () => { try { if (urlMode === 'encode') setUrlOutput(encodeURIComponent(urlInput)); else setUrlOutput(decodeURIComponent(urlInput)); } catch (e) { setUrlOutput('Error: Malformed URL sequence'); showToast('URL Error', 'error'); } };
  ...
  const createGif = () => { if(!gifVideo) return; setGifGenerating(true); gifshot.createGIF({ 'video': [URL.createObjectURL(gifVideo)], 'numFrames': gifFrames, 'gifWidth': gifW }, function(obj) { if(!obj.error) { setGifResult(obj.image); showToast('GIF Created Successfully'); } else { showToast('Error creating GIF', 'error'); } setGifGenerating(false); }); };
  ...
  const startMemo = async () => { try { const stream = await navigator.mediaDevices.getUserMedia({ audio: true }); setMemoStream(stream); memoRecorderRef.current = new MediaRecorder(stream); memoChunks.current = []; memoRecorderRef.current.ondataavailable = (e) => { if (e.data.size > 0) memoChunks.current.push(e.data); }; memoRecorderRef.current.onstop = () => { const blob = new Blob(memoChunks.current, { type: 'audio/webm' }); setMemoUrl(URL.createObjectURL(blob)); showToast('Voice Memo Saved'); }; memoRecorderRef.current.start(); setIsRecordingMemo(true); } catch (err) { showToast('Microphone access denied.', 'error'); } };
      ...
      <header className="header">
        <div className="header-content">
          ...
          <div className="search-container">
            ...
            <input 
              type="text" 
              placeholder="Search for a tool (e.g. Video Editor, Diff, CSV)..." 
          ...
          {activeTab === 'home' && (
            <div className="home-dashboard">
              <div className="home-hero">
                ...
                <p style={{color: 'var(--text-muted)', fontSize: '1.2rem'}}>52+ powerful web utilities running entirely on your local device.</p>
              </div>
            ...
            <div className="tool-workspace">
              ...
              {activeTab === 'bcrypt' && (
                <div>
                  ...
                  {bcryptHashOut && <textarea rows="3" readOnly className="form-control readonly-area" value={bcryptHashOut} />}
                </div>
              )}
                ...
                <div> 
                  ...
                  {compressedImage && ( 
                    <div className="results-grid"> 
                      ...
                      <div><h4>Compressed</h4><p>{(compressedImage.size / 1024 / 1024).toFixed(2)} MB</p><a href={URL.createObjectURL(compressedImage)} download={`compressed-${originalImage.name}`} className="btn btn-primary" style={{marginTop: '10px'}}><Download size={16} /> Download</a></div> 
                    </div> 
                  )} 
                </div> 
              )}
              ...
              {activeTab === 'vid2gif' && (
                <div>
                  <div className="file-input-wrapper"><input type="file" accept="video/*" onChange={(e) => { setGifVideo(e.target.files[0]); setGifResult(null); }} className="file-input" /></div>
                  <div className="responsive-grid form-group">
              ...
              {activeTab === 'json-csv' && (
                <div>
                  ...
                  <div className="form-group"><textarea rows="6" className="form-control" placeholder={j2cMode === 'json2csv' ? 'Paste JSON array here...' : 'Paste CSV text here...'} value={j2cInput} onChange={(e) => setJ2cInput(e.target.value)} /></div>
              ...
              {activeTab === 'url-encode' && (
                <div>
                  ...
                  <div className="form-group"><textarea rows="4" className="form-control" placeholder="Enter URL or string..." value={urlInput} onChange={(e) => setUrlInput(e.target.value)} /></div>
