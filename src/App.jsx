...
const ToolCard = ({ tool, navigate }) => (
  <a href={`/tool/${tool.id}`} onClick={(e) => { e.preventDefault(); navigate(`/tool/${tool.id}`); }} className="tool-card reveal-target">
...
const ToolHeader = ({ tool, navigate }) => (
  <div className="tool-header reveal-target">
...
export default function App() {
  ...
  // --- THE MAGIC SCROLL ANIMATION HOOK ---
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          // Removes visibility when scrolling away to animate again!
    ...
    const observeElements = () => {
      const targets = document.querySelectorAll('.reveal-target:not(.reveal-observed)');
  ...
  // --- TOOL STATES (Standard Synchronous Load - Stable) ---
  ...
  const generateBcrypt = () => { if (!bcryptPassInput) { showToast('Enter text', 'error'); return; } const salt = bcrypt.genSaltSync(10); setBcryptHashOut(bcrypt.hashSync(bcryptPassInput, salt)); showToast('Bcrypt Hash Generated'); };
  ...
  const downloadVideo = () => { const blob = new Blob(recordedChunks, { type: 'video/webm' }); const url = trackUrl(URL.createObjectURL(blob)); const a = document.createElement('a'); a.href = url; a.download = 'screen-recording.webm'; a.click(); setRecordedChunks([]); showToast('Downloaded'); };
  ...
  const createGif = () => { if (!gifVideo) return; setGifGenerating(true); gifshot.createGIF({ 'video': [URL.createObjectURL(gifVideo)], 'numFrames': gifFrames, 'gifWidth': gifW }, function(obj) { if (!obj.error) { setGifResult(trackUrl(obj.image)); showToast('GIF Generated'); } else { showToast('GIF Error', 'error'); } setGifGenerating(false); }); };
  ...
  const [mdInput, setMdInput] = useState('# Hello World\n\n**Bold Text**'); const mdOutput = DOMPurify.sanitize(marked.parse(mdInput));
  ...
  const compressDocs = async () => { if (zipFiles.length === 0) return; setZipping(true); const zip = new JSZip(); zipFiles.forEach(file => { const safeName = file.name.replace(/^.*[\\\/]/, '').replace(/[^a-zA-Z0-9.\-_]/g, '_'); zip.file(safeName || 'unnamed_file', file); }); const content = await zip.generateAsync({ type: 'blob' }); setZipUrl(trackUrl(URL.createObjectURL(content))); setZipping(false); showToast('ZIP Created'); };
  ...
  return (
    <div className="container">
      ...
      <header className="header reveal-target">
        ...
        <main className="main-content">
          ...
          {activeTab === 'home' && (
            <div className="home-dashboard">
              <div className="home-hero reveal-target">
              ...
              {!searchQuery && (
                <>
                  <div className="section-heading reveal-target"><Sparkles size={24} color="var(--primary)" /> Popular Tools</div>
                  <div className="responsive-grid">
                    {popularToolIds.map(id => { const t = flatTools.find(tool => tool.id === id); return t ? <ToolCard key={t.id} tool={t} navigate={navigate} /> : null; })}
                  </div>
                </>
              )}

              <div className="section-heading reveal-target"><Layers size={24} color="var(--primary)" /> {searchQuery ? 'Search Results' : 'Browse Categories'}</div>
              {Object.keys(categories).filter(cat => !searchQuery || categories[cat].some(t => t.name.toLowerCase().includes(searchQuery.toLowerCase()) || t.description.toLowerCase().includes(searchQuery.toLowerCase()))).length === 0 ? (
                <div className="reveal-target" style={{textAlign: 'center', padding: '40px', color: 'var(--text-muted)'}}>No tools found for "{searchQuery}".</div>
              ) : (
                Object.keys(categories).map(cat => {
                  ...
                  return (
                    <div key={cat} style={{marginBottom: '40px'}}>
                      <h3 className="reveal-target" style={{marginBottom: '15px', color: 'var(--text-main)'}}>{cat}</h3>
                      <div className="responsive-grid">
      ...
      <footer className="reveal-target" style={{ textAlign: 'center', padding: '60px 20px 40px 20px', color: 'var(--text-muted)', fontSize: '0.95rem', borderTop: '1px solid var(--border)', background:'var(--bg-surface)' }}>
      ...
      {activeModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 23, 42, 0.7)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px', animation:'fadeIn 0.2s ease-out' }} onClick={() => setActiveModal(null)}>
          <div style={{ background: 'var(--bg-surface)', padding: '50px', borderRadius: '24px', maxWidth: '650px', width: '100%', maxHeight: '85vh', overflowY: 'auto', position: 'relative', boxShadow: 'var(--shadow-lg)' }} onClick={e => e.stopPropagation()}>
