...
import DOMPurify from 'dompurify'; // SECURITY: Added for XSS sanitization
...
const categories = {
  ...
  "Writing & Text": [
    ...
    { id: 'md', name: 'Markdown', icon: FileCode2, description: 'Render and preview Markdown syntax in real-time.' },
  ],
  "Marketing & Business": [
  ...
  "General Utilities": [
    { id: 'zip', name: 'Zip Docs', icon: FileArchive, description: 'Compress files into a single .zip archive.' },
    { id: 'voicememo', name: 'Voice Memo', icon: MicVocal, description: 'Record and save audio notes in the browser.' },
...
const ToolHeader = ({ tool, navigate }) => (
  <div className="tool-header">
    <button className="back-btn" onClick={() => navigate('/')}><ArrowLeft size={16}/> Browse all tools</button>
    <h1>{tool.name}</h1>
    <p>{tool.description}</p>
  </div>
);
...
export default function App() {
  ...
  // --- SECURITY: File Size Validation Helper ---
  ...
  const handleApiSend = async () => {
    ...
    try {
      let headers = {};
      if (apiHeaders.trim()) { try { headers = JSON.parse(apiHeaders); } catch { throw new Error('Invalid JSON in Headers'); } }
      ...
      try { setApiResponse(JSON.stringify(JSON.parse(text), null, 2)); } catch { setApiResponse(text); }
  ...
  useEffect(() => () => { if (codeImgUrl) URL.revokeObjectURL(codeImgUrl); }, [codeImgUrl]);
  ...
  const generateCodeImage = () => {
    ...
    if (codeImgUrl) URL.revokeObjectURL(codeImgUrl);
    setCodeImgUrl(canvas.toDataURL('image/png'));
    showToast('Code Image Rendered!');
  };
  ...
  useEffect(() => () => { if (convUrl) URL.revokeObjectURL(convUrl); }, [convUrl]);
  ...
  useEffect(() => () => { if (favZipUrl) URL.revokeObjectURL(favZipUrl); }, [favZipUrl]);
  ...
  const generateFavicons = async () => {
    ...
    const sizes = [
      ...
      { name: 'favicon-48x48.png', size: 48 }, { name: 'apple-touch-icon.png', size: 180 },
      { name: 'android-chrome-192x192.png', size: 192 }, { name: 'android-chrome-512x512.png', size: 512 }
    ];
    const img = new window.Image();
    ...
    for (const item of sizes) {
      ...
      const ctx = canvas.getContext('2d'); ctx.drawImage(img, 0, 0, item.size, item.size);
    ...
    if (favZipUrl) URL.revokeObjectURL(favZipUrl);
    setFavZipUrl(URL.createObjectURL(content));
    setFavGenerating(false);
    showToast('Favicon Bundle Created!');
  };
  ...
  useEffect(() => () => { if (stegResultUrl) URL.revokeObjectURL(stegResultUrl); }, [stegResultUrl]);
  ...
  const handleStegProcess = () => {
    ...
    img.onload = () => {
      ...
      if (stegMode === 'encode') {
        const text = stegSecret; let bits = ''; const len = text.length;
        for (let i = 31; i >= 0; i--) bits += (len >> i) & 1;
        for (let i = 0; i < len; i++) { const code = text.charCodeAt(i); for (let j = 7; j >= 0; j--) bits += (code >> j) & 1; }
        if (bits.length > data.length) { showToast('Image too small for this message', 'error'); return; }
        for (let i = 0; i < bits.length; i++) { data[i] = (data[i] & ~1) | parseInt(bits[i], 10); }
        ctx.putImageData(imgData, 0, 0);
        if (stegResultUrl) URL.revokeObjectURL(stegResultUrl);
        setStegResultUrl(canvas.toDataURL('image/png'));
        showToast('Secret Encoded into Image');
      } else {
        let lenBits = ''; for (let i = 0; i < 32; i++) lenBits += data[i] & 1;
        const msgLen = parseInt(lenBits, 2);
        if (isNaN(msgLen) || msgLen <= 0 || msgLen > 100000) {
          setStegDecoded('No hidden message found or unsupported format.');
          showToast('No Hidden Message', 'error'); return;
        }
        let decoded = ''; let bitIndex = 32;
        for (let i = 0; i < msgLen; i++) {
          let charBits = ''; for (let j = 0; j < 8; j++) { charBits += data[bitIndex++] & 1; }
          decoded += String.fromCharCode(parseInt(charBits, 2));
        }
        setStegDecoded(decoded);
        showToast('Hidden Message Decoded!');
      }
      URL.revokeObjectURL(img.src);
    };
  ...
  const handleComputeFileHash = async () => {
    if (!hashFile) return;
    if (!validateFile(hashFile, 500)) return; // 500MB limit for browser hashing memory
    setHashingFile(true);
    try {
      ...
      showToast('Failed to hash file', 'error');
    }
  ...
  const handlePgpProcess = async () => {
    ...
    try {
      if (pgpMode === 'encrypt') {
        ...
        showToast('PGP Message Encrypted');
      } else {
  ...
  const testRegex = () => { try { const re = new RegExp(regexPattern, 'g'); const matches = regexText.match(re); setRegexResult(matches ? matches.join(', ') : 'No matches found.'); } catch { setRegexResult('Error: Invalid Regex Pattern'); showToast('Invalid Regex', 'error'); } };
  ...
  const [compressedImgUrl, setCompressedImgUrl] = useState(null); // SECURITY FIX: Memory Management
  const [compressing, setCompressing] = useState(false); const [targetSize, setTargetSize] = useState(''); const [targetUnit, setTargetUnit] = useState('KB'); const [compressError, setCompressError] = useState('');
  useEffect(() => () => { if (compressedImgUrl) URL.revokeObjectURL(compressedImgUrl); }, [compressedImgUrl]);

  const handleImageUpload = (e) => { const file = e.target.files[0]; if (validateFile(file, 50)) { setOriginalImage(file); setCompressedImage(null); setCompressError(''); } };
  const handleCompressImage = async () => { if (!originalImage || !targetSize || targetSize <= 0) { showToast('Enter valid size', 'error'); return; } setCompressing(true); try { const sizeInMB = targetUnit === 'KB' ? targetSize / 1024 : Number(targetSize); const options = { maxSizeMB: sizeInMB, maxWidthOrHeight: 4000, useWebWorker: true }; const compressedFile = await imageCompression(originalImage, options); setCompressedImage(compressedFile); if (compressedImgUrl) URL.revokeObjectURL(compressedImgUrl); setCompressedImgUrl(URL.createObjectURL(compressedFile)); showToast('Compressed!'); } catch { showToast('Failed compression', 'error'); } finally { setCompressing(false); } };
  
  const [text, setText] = useState(''); const words = text.trim() ? text.trim().split(/\s+/).length : 0; const chars = text.length;
  const [caseText, setCaseText] = useState('');
  
  // SECURITY FIX: Replaced Math.random with Web Crypto API
  ...
  const formatJson = () => { try { setJsonOutput(JSON.stringify(JSON.parse(jsonInput), null, 2)); showToast('JSON Formatted'); } catch { setJsonOutput('Error: Invalid JSON'); showToast('Invalid JSON', 'error'); } };
  ...
  const compressDocs = async () => { 
    ...
    zipFiles.forEach(file => {
      // SECURITY FIX: Sanitize filename to prevent ZIP path traversal
  ...
  useEffect(() => () => { if (resizedDataUrl) URL.revokeObjectURL(resizedDataUrl); }, [resizedDataUrl]);
  const handleResize = () => { 
    if (!resizeSource) return; 
    const img = new window.Image(); 
    img.onload = () => { 
      const canvas = document.createElement('canvas'); canvas.width = targetWidth; canvas.height = img.height * (targetWidth / img.width); 
  ...
  const [mongoId, setMongoId] = useState(''); const [mongoResult, setMongoResult] = useState(''); const extractMongoDate = () => { if (mongoId.length === 24) setMongoResult(new Date(parseInt(mongoId.substring(0, 8), 16) * 1000).toLocaleString()); else { setMongoResult('Invalid ObjectId'); showToast('Invalid ID', 'error'); } };
  
  const [jwt, setJwt] = useState(''); const [jwtData, setJwtData] = useState(''); 
  const decodeJwt = () => { try { setJwtData(JSON.stringify(JSON.parse(atob(jwt.split('.')[1].replace(/-/g, '+').replace(/_/g, '/'))), null, 2)); } catch { setJwtData('Invalid JWT Format'); showToast('Invalid JWT', 'error'); } };
  
  const [mdInput, setMdInput] = useState('# Hello World\n\n**Bold Text**'); 
  // SECURITY FIX: Added DOMPurify
  ...
  const startRecording = async () => { try { const stream = await navigator.mediaDevices.getDisplayMedia({ video: true }); mediaRecorderRef.current = new MediaRecorder(stream); mediaRecorderRef.current.ondataavailable = (e) => { if (e.data.size > 0) setRecordedChunks(prev => [...prev, e.data]); }; mediaRecorderRef.current.start(); setIsRecording(true); stream.getVideoTracks()[0].onended = () => stopRecording(); } catch {} }; 
  const stopRecording = () => { if (mediaRecorderRef.current) mediaRecorderRef.current.stop(); setIsRecording(false); }; 
  const downloadVideo = () => { const blob = new Blob(recordedChunks, { type: 'video/webm' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'screen-recording.webm'; a.click(); setRecordedChunks([]); showToast('Downloaded'); URL.revokeObjectURL(url); };
  ...
  const convertSvg = () => { const blob = new Blob([svgInput], { type: 'image/svg+xml;charset=utf-8' }); const img = new window.Image(); img.onload = () => { const canvas = document.createElement('canvas'); canvas.width = img.width; canvas.height = img.height; canvas.getContext('2d').drawImage(img, 0, 0); if (pngUrl) URL.revokeObjectURL(pngUrl); setPngUrl(canvas.toDataURL('image/png')); showToast('Converted'); URL.revokeObjectURL(img.src); }; img.src = URL.createObjectURL(blob); };
  ...
  const handleUrlTransform = () => { try { if (urlMode === 'encode') setUrlOutput(encodeURIComponent(urlInput)); else setUrlOutput(decodeURIComponent(urlInput)); } catch { setUrlOutput('Error: Malformed URL sequence'); showToast('URL Error', 'error'); } };
  
  const [paletteColors, setPaletteColors] = useState([]);
  const handlePaletteUpload = (e) => { const file = e.target.files[0]; if (!validateFile(file, 25)) return; const img = new window.Image(); img.onload = () => { const canvas = document.createElement('canvas'); const ctx = canvas.getContext('2d'); canvas.width = 100; canvas.height = 100; ctx.drawImage(img, 0, 0, 100, 100); const data = ctx.getImageData(0, 0, 100, 100).data; const sampled = []; for (let i = 0; i < data.length; i += 400) { const r = data[i].toString(16).padStart(2, '0'); const g = data[i+1].toString(16).padStart(2, '0'); const b = data[i+2].toString(16).padStart(2, '0'); sampled.push(`#${r}${g}${b}`); } const unique = [...new Set(sampled)].slice(0, 6); setPaletteColors(unique); showToast('Palette Extracted'); URL.revokeObjectURL(img.src); }; img.src = URL.createObjectURL(file); };
  ...
  useEffect(() => () => { if (exportedAudioUrl) URL.revokeObjectURL(exportedAudioUrl); }, [exportedAudioUrl]);
      ...
      <header className="header">
        <div className="header-content">
          ...
          <div className="search-container">
            ...
            <input type="text" placeholder="Search across 62+ tools (e.g. API Tester, PGP, JSON)..." className="search-bar" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
          </div>
        ...
        <main className="main-content">
          ...
          {activeTab !== 'home' && currentTool && (
            <div className="tool-workspace">
              ...
              {activeTab === 'svg' && ( <div> <div className="form-group"><textarea rows="6" className="form-control" value={svgInput} onChange={(e) => setSvgInput(e.target.value)} /></div> <button onClick={convertSvg} className="btn btn-primary" style={{marginBottom:'15px'}}><RefreshCw size={16}/> Convert</button> {pngUrl && <div><img src={pngUrl} alt="Converted PNG" style={{display:'block', marginBottom:'15px', borderRadius:'8px'}}/><a href={pngUrl} download="converted.png" className="btn btn-secondary"><Download size={16}/> Download PNG</a></div>} </div> )}
              {activeTab === 'svg-minify' && ( <div> <div className="form-group"><textarea rows="6" className="form-control" value={svgMinInput} onChange={(e) => setSvgMinInput(e.target.value)} /></div> <button onClick={minifySvg} className="btn btn-primary" style={{marginBottom:'15px'}}><FileCode2 size={16}/> Minify SVG</button> {svgSavings && <p style={{ color: 'var(--success)', fontWeight: '600', marginBottom: '10px' }}>{svgSavings}</p>} {svgMinOutput && <div className="form-group"><textarea rows="6" readOnly className="form-control readonly-area" value={svgMinOutput} /></div>} </div> )}
              ...
              {activeTab === 'json-csv' && ( <div> <div className="btn-group"> <button className={`btn ${j2cMode === 'json2csv' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setJ2cMode('json2csv')}>JSON to CSV</button> <button className={`btn ${j2cMode === 'csv2json' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setJ2cMode('csv2json')}>CSV to JSON</button> </div> <div className="form-group"><textarea rows="6" className="form-control" placeholder={j2cMode === 'json2csv' ? 'Paste JSON array here...' : 'Paste CSV text here...'} value={j2cInput} onChange={(e) => setJ2cInput(e.target.value)} /></div> <button onClick={runJ2c} className="btn btn-primary form-group"><FileJson size={16}/> Convert Format</button> {j2cOutput && <div className="form-group"><label>Output</label><textarea rows="8" readOnly className="form-control readonly-area" value={j2cOutput} /></div>} </div> )}
              ...
              {activeTab === 'url-encode' && ( <div> <div className="btn-group"> <button className={`btn ${urlMode === 'encode' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setUrlMode('encode')}>Encode</button> <button className={`btn ${urlMode === 'decode' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setUrlMode('decode')}>Decode</button> </div> <div className="form-group"><textarea rows="4" className="form-control" placeholder="Enter URL or string..." value={urlInput} onChange={(e) => setUrlInput(e.target.value)} /></div> <button onClick={handleUrlTransform} className="btn btn-primary form-group"><Link size={16}/> {urlMode === 'encode' ? 'Encode URL' : 'Decode URL'}</button> {urlOutput && <div className="form-group"><label>Output</label><textarea rows="4" readOnly className="form-control readonly-area" value={urlOutput} /></div>} </div> )}
              ...
              {/* SECURITY FIX: Added disclaimer to JWT Decoder */}
              ...
              {/* Writing & Text */}
              {activeTab === 'counter' && ( <div> <div className="form-group"><textarea rows="6" className="form-control" value={text} onChange={(e) => setText(e.target.value)} /></div> <div className="results-grid"> <div className="stat-box"><h4>Words</h4><p>{words}</p></div> <div className="stat-box"><h4>Characters</h4><p>{chars}</p></div> </div> </div> )}
              ...
              {activeTab === 'md' && ( <div> <div className="form-group"><textarea rows="6" className="form-control" value={mdInput} onChange={(e) => setMdInput(e.target.value)} /></div> <div className="form-group"><label>Live Preview</label> <div style={{padding:'20px', background:'var(--bg-surface)', border:'1px solid var(--border)', borderRadius:'12px'}} dangerouslySetInnerHTML={{ __html: mdOutput }} /> </div> </div> )}
              ...
              {activeTab === 'invoice' && ( <div> <div className="form-group"><label>Client Name</label><input type="text" className="form-control" placeholder="John Doe" value={client} onChange={(e) => setClient(e.target.value)} /></div> <button onClick={generateInvoice} className="btn btn-primary"><Download size={16}/> Download PDF Invoice</button> </div> )}
              {activeTab === 'dummy-data' && ( <div> <div className="form-group"><label>Records Count (1 - 50): {dummyCount}</label><input type="range" min="1" max="50" value={dummyCount} onChange={(e) => setDummyCount(e.target.value)} /></div> <div className="btn-group"> <button className={`btn ${dummyFormat === 'json' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setDummyFormat('json')}>JSON</button> <button className={`btn ${dummyFormat === 'csv' ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setDummyFormat('csv')}>CSV</button> </div> <button onClick={generateMockData} className="btn btn-primary form-group"><Layers size={16}/> Generate Records</button> {dummyOutput && <div className="form-group"><label>Output</label><textarea rows="8" readOnly className="form-control readonly-area" value={dummyOutput} /></div>} </div> )}
              ...
              {activeTab === 'utm' && ( <div> <div className="form-group"><label>Website URL</label><input type="text" className="form-control" value={utmUrl} onChange={(e) => setUtmUrl(e.target.value)} /></div> <div className="responsive-grid form-group"> <div><label>Source</label><input type="text" className="form-control" value={utmSrc} onChange={(e) => setUtmSrc(e.target.value)} /></div> <div><label>Medium</label><input type="text" className="form-control" value={utmMed} onChange={(e) => setUtmMed(e.target.value)} /></div> <div><label>Campaign</label><input type="text" className="form-control" value={utmCamp} onChange={(e) => setUtmCamp(e.target.value)} /></div> </div> <div className="form-group"><label>Generated UTM Link</label><div className="form-control readonly-area">{utmResult}</div></div> </div> )}
              {activeTab === 'qr' && ( <div> <div className="form-group"><label>URL or Text</label><input type="text" className="form-control" value={qrText} onChange={(e) => setQrText(e.target.value)} /></div> <div style={{background:'#fff', padding:'20px', borderRadius:'12px', display:'inline-block'}}><QRCodeCanvas value={qrText} size={200} level={"H"} /></div> </div> )}

              {/* General Utilities */}
              {activeTab === 'zip' && ( <div> <div className="file-input-wrapper"><input type="file" multiple onChange={(e) => { setZipFiles(Array.from(e.target.files)); }} className="file-input" /></div> <button onClick={compressDocs} disabled={zipFiles.length === 0 || zipping} className="btn btn-primary form-group">{zipping ? 'Compressing...' : 'Create ZIP'}</button> {zipUrl && <div style={{marginTop: '20px'}}><a href={zipUrl} download="archive.zip" className="btn btn-secondary">Download ZIP</a></div>} </div> )}
          ...
          {activeTab !== 'home' && currentTool && (
            <div className="seo-content" style={{ marginTop: '40px', padding: '30px', borderTop: '1px solid var(--border)', color: 'var(--text-main)' }}>
              ...
              <p style={{ color:'var(--text-muted)' }}>
                Unlike traditional online utilities, our {currentTool.name} runs 100% locally within your browser. Your data is never collected, stored, or transmitted to external servers. Enjoy secure processing without compromising your privacy.
              </p>
            </div>
          )}
        </main>
      ...
      {activeModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' }} onClick={() => setActiveModal(null)}>
          <div style={{ background: '#ffffff', padding: '40px', borderRadius: '24px', maxWidth: '600px', width: '100%', maxHeight: '85vh', overflowY: 'auto', position: 'relative', boxShadow: 'var(--shadow-lg)' }} onClick={e => e.stopPropagation()}>
            ...
            {activeModal === 'privacy' && ( <> <h2 style={{ marginBottom: '20px', fontSize: '2rem' }}>Privacy Policy</h2> <p style={{ marginBottom: '15px', color: 'var(--text-muted)' }}>At I Love Tools, your privacy is our extreme priority. All tools and utilities provided on this website operate 100% client-side. We do not upload, process, or store any of your files, images, code, or text on external servers.</p> </> )}
            {activeModal === 'terms' && ( <> <h2 style={{ marginBottom: '20px', fontSize: '2rem' }}>Terms of Service</h2> <p style={{ marginBottom: '15px', color: 'var(--text-muted)' }}>All tools provided on this website are free to use and run entirely locally in your browser. We provide these utilities "as is" without any warranties of any kind.</p> </> )}
          </div>
        </div>
      )}
