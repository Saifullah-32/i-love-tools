...
const ToolHeader = ({ tool, navigate }) => ( <div className="tool-header"> <button className="back-btn" onClick={() => navigate('/')}><ArrowLeft size={16}/> Browse all tools</button> <h1>{tool.name}</h1> <p>{tool.description}</p> </div> );
const RelatedTools = ({ currentTool, navigate }) => { let catName = Object.keys(categories).find(cat => categories[cat].some(t => t.id === currentTool.id)); if (!catName) return null; const related = categories[catName].filter(t => t.id !== currentTool.id).slice(0, 3); if (related.length === 0) return null; return ( <div style={{marginTop: '3rem', paddingTop: '2rem', borderTop: '1px solid var(--border)'}}> <h3 style={{marginBottom: '1rem', fontSize: '1.2rem'}}>Related Tools</h3> <div className="responsive-grid"> {related.map(t => <ToolCard key={t.id} tool={t} navigate={navigate} />)} </div> </div> ); };
...
export default function App() {
  ...
  const navigate = (path) => { window.history.pushState({}, '', path); setCurrentPath(path); setSearchQuery(''); setActiveDropdown(null); window.scrollTo(0, 0); };
  ...
  useEffect(() => {
    ...
    let link = document.querySelector('link[rel="canonical"]'); if (!link) { link = document.createElement('link'); link.setAttribute('rel', 'canonical'); document.head.appendChild(link); } link.setAttribute('href', canonical);
  ...
  const [mdInput, setMdInput] = useState('# Hello World\n\n**Bold Text**'); const [mdOutput, setMdOutput] = useState(''); useEffect(() => { if (activeTab === 'md') { (async () => { try { const { marked } = await import('marked'); const DOMPurify = (await import('dompurify')).default; setMdOutput(DOMPurify.sanitize(marked.parse(mdInput))); } catch (e) {} })(); } }, [mdInput, activeTab]);
        ...
        <main className="main-content">
          ...
          {activeTab !== 'home' && currentTool && (
            <div className="tool-workspace">
              ...
              {activeTab === 'hash' && ( <div> <div className="form-group"><label>Text to Hash</label><textarea rows="4" className="form-control" value={hashData} onChange={(e) => setHashData(e.target.value)} /></div> <button onClick={generateHash} className="btn btn-primary form-group">Generate Hash</button> {hashResult && <div className="form-control readonly-area"><code>{hashResult}</code></div>} </div> )}
