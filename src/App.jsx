...
import { 
  ...
  Send, Sliders, Eye, FileCheck, Shield, Lock, FileCode, Check
} from 'lucide-react';
...
export default function App() {
  ...
  const handleApiSend = async () => {
    ...
    try {
      let headers = {};
      if (apiHeaders.trim()) {
        try { headers = JSON.parse(apiHeaders); } catch { throw new Error('Invalid JSON in Headers'); }
      }
  ...
  const generateCodeImage = () => {
    ...
    const cardX = padX;
    const cardY = padY;
    const cardW = canvas.width - padX * 2;
    const cardH = canvas.height - padY * 2;
    ...
    lines.forEach((line, i) => {
      ...
      ctx.fillText(String(i + 1).padStart(2, ' '), cardX + 20, cardY + 60 + i * lineHeight);
      ctx.fillStyle = '#cdd6f4';
      ctx.fillText(line, cardX + 55, cardY + 60 + i * lineHeight);
    });
  ...
  const [stegSecret, setStegSecret] = useState('Confidential message 123');
    ...
    img.onload = () => {
      ...
      if (stegMode === 'encode') {
        ...
        for (let i = 0; i < bits.length; i++) {
          data[i] = (data[i] & ~1) | parseInt(bits[i], 10);
        }
        ...
        if (isNaN(msgLen) || msgLen <= 0 || msgLen > 100000) {
          ...
          showToast('No Hidden Message', 'error');
          return;
        }
  ...
  const [pgpMsg, setPgpMsg] = useState('My top secret message');
  const [pgpPass, setPgpPass] = useState('SuperSecretPassphrase123');
  const [pgpOutput, setPgpOutput] = useState('');
  const [pgpProcessing, setPgpProcessing] = useState(false);

  const handlePgpProcess = async () => {
    if (!pgpMsg || !pgpPass) {
      showToast('Message and Passphrase required', 'error');
      return;
    }
  ...
  const compressDocs = async () => { if (zipFiles.length === 0) return; setZipping(true); const zip = new JSZip(); zipFiles.forEach(file => zip.file(file.name, file)); const content = await zip.generateAsync({ type: 'blob' }); setZipUrl(URL.createObjectURL(content)); setZipping(false); showToast('ZIP Created'); };
  ...
  const downloadVideo = () => { const blob = new Blob(recordedChunks, { type: 'video/webm' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'screen-recording.webm'; a.click(); setRecordedChunks([]); showToast('Downloaded'); };
  ...
  const convertSvg = () => { const blob = new Blob([svgInput], { type: 'image/svg+xml;charset=utf-8' }); const img = new window.Image(); img.onload = () => { const canvas = document.createElement('canvas'); canvas.width = img.width; canvas.height = img.height; canvas.getContext('2d').drawImage(img, 0, 0); setPngUrl(canvas.toDataURL('image/png')); showToast('Converted'); }; img.src = URL.createObjectURL(blob); };
  ...
  const runDiff = () => { setDiffResult(diffLines(diffA, diffB)); showToast('Diff Computed'); };
  ...
  const createGif = () => { if (!gifVideo) return; setGifGenerating(true); gifshot.createGIF({ 'video': [URL.createObjectURL(gifVideo)], 'numFrames': gifFrames, 'gifWidth': gifW }, function(obj) { if (!obj.error) { setGifResult(obj.image); showToast('GIF Generated'); } else { showToast('GIF Error', 'error'); } setGifGenerating(false); }); };
      ...
      <header className="header">
        <div className="header-content">
          ...
          <div className="search-container">
            ...
            <input 
              type="text" 
              placeholder="Search across 62+ tools (e.g. API Tester, PGP, Steganography)..." 
            ...
            <div className="tool-workspace">
              ...
              {activeTab === 'pgp-tool' && (
                <div>
                  ...
                  <div className="form-group"><label>Passphrase</label><input type="password" className="form-control" value={pgpPass} onChange={(e) => setPgpPass(e.target.value)} /></div>
              ...
              {/* PRE-EXISTING TOOLS (43 THROUGH 52) */}
              ...
              {activeTab === 'uuid-gen' && ( <div> <div className="form-group"><label>Count (1 - 100): {uuidCount}</label><input type="range" min="1" max="100" value={uuidCount} onChange={(e) => setUuidCount(e.target.value)} /></div> <button onClick={generateUuids} className="btn btn-primary form-group"><Key size={16}/> Generate UUIDs</button> {uuidOutput && <div className="form-group"><label>Generated UUIDs</label><textarea rows="8" readOnly className="form-control readonly-area" value={uuidOutput} /></div>} </div> )}
              ...
              {activeTab === 'jwt' && ( <div> <div className="form-group"><label>JSON Web Token</label><textarea rows="4" className="form-control" placeholder="Paste JWT here..." value={jwt} onChange={(e) => setJwt(e.target.value)} /></div> <button onClick={decodeJwt} className="btn btn-primary form-group"><LockOpen size={16}/> Decode Token</button> <div className="form-group"><label>Decoded Payload</label><textarea rows="6" readOnly className="form-control readonly-area" value={jwtData} /></div> </div> )}
